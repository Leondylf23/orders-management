...
const allOrders = async (request, reply) => {
    try {
        ...
        const response = await OrderinAjaHelper.getAllOrder(userData?.userId, false);
...
const productDetail = async (request, reply) => {
    try {
        ...
        const formData = request.query;
        const response = await OrderinAjaHelper.getProductDetail(formData, null, false);
...
const allMyProducts = async (request, reply) => {
    try {
        ...
        const formData = request.query;
        const response = await OrderinAjaHelper.getAllProducts(formData, userData?.userId);
...
const myProductDetail = async (request, reply) => {
    try {
        ...
        const formData = request.query;
        const response = await OrderinAjaHelper.getProductDetail(formData, userData?.userId, true);
        ...
        console.log([fileName, 'My Product Detail API', 'ERROR'], { info: `${err}` });
        return reply.send(GeneralHelper.errorResponse(err));
    }
};
...
const createProduct = async (request, reply) => {
    try {
        ...
        const response = await OrderinAjaHelper.addProduct({ ...formData, imageData: imageFile[0] }, userData?.userId);
...
const createOrder = async (request, reply) => {
    try {
        ...
        const response = await OrderinAjaHelper.addOrder(decryptData, userData?.userId);
...
const editProduct = async (request, reply) => {
    try {
        ...
        const formData = request.body;
        const response = await OrderinAjaHelper.editProductData({...formData, imageData: imageFile ? imageFile[0] : null}, userData?.userId);
        ...
        console.log([fileName, 'Edit Product Data API', 'ERROR'], { info: `${err}` });
        return reply.send(GeneralHelper.errorResponse(err));
    }
};
...
const updateOrderStatus = async (request, reply) => {
    try {
        ...
        const formData = request.body;
        const response = await OrderinAjaHelper.updateStatusOrder(formData, userData?.userId);
        ...
        console.log([fileName, 'Delete Product Data API', 'ERROR'], { info: `${err}` });
        return reply.send(GeneralHelper.errorResponse(err));
    }
};
...
const deleteProduct = async (request, reply) => {
    try {
        ...
        const formData = request.body;
        const response = await OrderinAjaHelper.deleteProduct(formData, userData?.userId);
...
Router.get('/order/business/detail', AuthMiddleware.validateToken, getBusinessOrderDetail);
Router.get('/product/myproducts', AuthMiddleware.validateToken, allMyProducts);
Router.get('/product/myproducts/detail', AuthMiddleware.validateToken, myProductDetail);

Router.put('/product/create', AuthMiddleware.validateToken, MulterMiddleware.fields([{ name: 'imageData', maxCount: 1 }]), createProduct);

Router.patch('/product/edit', AuthMiddleware.validateToken, MulterMiddleware.fields([{ name: 'imageData', maxCount: 1 }]), editProduct);
Router.patch('/order/status/update', AuthMiddleware.validateToken, updateOrderStatus);
